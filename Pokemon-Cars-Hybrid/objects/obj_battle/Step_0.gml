		...
		else
		{
			...
			if (showItemsOptions)
			{
				...
				if (keyboard_check_pressed(vk_enter))
				{
					...
					if (item_option == 0)
					{
						var refuelItem = irandom_range(0,2);
						
						if (refuelItem == 0)
						{
							ds_messages [| 0] = "Player uses " + a_item_text[item_option];
							ds_messages [| 1] = "and refuels " + a_attack_text[refuelItem];
							attackMoveOne += 3;
						}
						if (refuelItem == 1)
						{
					...
					if (item_option == 2)
					{
						if(global.playerHP < playerMAXHP)
						{
						ds_messages [| 0] = "Player calls upon " + a_item_text[item_option];
						...
						{
							ds_messages [| 0] = "Can't heal! Already max hp!"
						}
					}
